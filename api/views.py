...
def convert_similarity_array(similarity: list[list[float]], is_bulk: bool = False) -> Response:
    print("similarity=", len(similarity))
    data = [{"score": similarity[0]} for similarity in similarity]
    response = serializers.JudgeResultSerializer(data, many=True) if is_bulk else serializers.JudgeResultSerializer(data[0], many=False)
...
def judge(request):
    ...
    if isinstance(request_data, serializers.JudgeSentence):
        similarity = ST.determine_similarity([request_data.sentence1], [request_data.sentence2])
...
def judge_bulk(request):
    ...
    serializer = serializers.JudgeSentenceSerializer(data=request.data, many=True, max_length=100)
    if not serializer.is_valid():
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    request_data = serializer.save()
